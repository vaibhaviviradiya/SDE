...
exports.getAllUsers = async (req, res) => {
  try {
    const users = await User.find().select('companyName ownerName email phone address role trustScore');  
    res.json(users);
  } catch (err) {
    res.status(500).json({ message: "Server Error" });
  }
...
exports.getuserbyrole = async (req, res) => {
  try {
    ...
    const users = await User.find({ role }).select('companyName ownerName email phone role trustScore');
    res.json(users);
  } catch (err) {
    res.status(500).json({ message: "Server Error" });
  }
